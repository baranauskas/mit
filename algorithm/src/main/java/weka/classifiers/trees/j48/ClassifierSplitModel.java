...
public abstract class ClassifierSplitModel
  ...
  public final String dumpLabel(int index,Instances data) throws Exception {
   ...
   public final String dumpLabelNormalized(int index, Instances data, int forestSize) throws Exception {
    ...
    text.append(((Instances)data).classAttribute().
		value(m_distribution.maxClass(index)));
    text.append(" ("+Utils.roundDouble(m_distribution.perBag(index) / forestSize,2));
    if (Utils.gr(m_distribution.numIncorrect(index),0))
      text.append("/"+Utils.roundDouble(m_distribution.numIncorrect(index) / forestSize,2));
    text.append(")");
