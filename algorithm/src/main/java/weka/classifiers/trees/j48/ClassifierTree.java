...
public class ClassifierTree implements Drawable, Serializable, RevisionHandler, CapabilitiesHandler {
  ...
  public String graphNormalized(int forestSize) throws Exception {
    ...
    if (m_isLeaf) {
      text.append("N" + m_id + " [label=\""
        + Utils.backQuoteChars(m_localModel.dumpLabelNormalized(0, m_train, forestSize)) + "\" "
        + "shape=box style=filled ");
      if (m_train != null && m_train.numInstances() > 0) {
        text.append("data =\n" + m_train + "\n");
        text.append(",\n");
      ...
      text.append("]\n");
      graphTreeNormalized(text, forestSize);
    }
  ...
  public String toString() {
    ...
    public String toStringNormalized(int forestSize) {
    ...
    try {
      ...
      if (m_isLeaf) {
        text.append(": ");
        text.append(m_localModel.dumpLabelNormalized(0, m_train, forestSize));
      } else {
        dumpTreeNormalized(0, text, forestSize);
      }
      text.append("\n\nNumber of Leaves  : \t" + numLeaves() + "\n");
      text.append("\nSize of the tree : \t" + numNodes() + "\n");
  ...
  private void dumpTreeNormalized(int depth, StringBuffer text, int forestSize) throws Exception {
    ...
    for (i = 0; i < m_sons.length; i++) {
      ...
      if (m_sons[i].m_isLeaf) {
        text.append(": ");
        text.append(m_localModel.dumpLabelNormalized(i, m_train, forestSize) );
      } else {
        m_sons[i].dumpTreeNormalized(depth + 1, text, forestSize);
      }
    }
  }
  ...
  private void graphTree(StringBuffer text) throws Exception {
   ...
   private void graphTreeNormalized(StringBuffer text, int forestSize) throws Exception {
    ...
    for (int i = 0; i < m_sons.length; i++) {
      ...
      if (m_sons[i].m_isLeaf) {
        text.append("N" + m_sons[i].m_id + " [label=\""
          + Utils.backQuoteChars(m_localModel.dumpLabelNormalized(i, m_train, forestSize)) + "\" "
          + "shape=box style=filled ");
        if (m_train != null && m_train.numInstances() > 0) {
          text.append("data =\n" + m_sons[i].m_train + "\n");
          text.append(",\n");
        ...
        text.append("]\n");
        m_sons[i].graphTreeNormalized(text, forestSize);
      }
    }
  }
